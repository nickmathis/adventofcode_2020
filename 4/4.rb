...
class AoC3
  ...
  def p1
    valid = 0
    @passports.each { |p| valid += 1 if (p.keys - [:cid]).count == 7 }
    valid
  end
  ...
  def p2
    ...
    @passports.each do |p|
      ...
      if (p.keys - [:cid]).count == 7 &&
        ...
        # puts "byr: #{byr_valid?(p[:byr])} iyr: #{iyr_valid?(p[:iyr])} eyr: #{eyr_valid?(p[:eyr])} hgt: #{hgt_valid?(p[:hgt])} hcl: #{hcl_valid?(p[:hcl])} ecl: #{ecl_valid?(p[:ecl])} pid: #{pid_valid?(p[:pid])}"
      end
    end
    valid
  end
  ...
  def hgt_valid?(val)
    m = val && val.match(/(\d+)(in|cm)/)
    m && ((m[2] == 'cm' && m[1].to_i >= 150 && m[1].to_i <= 193) || m[2] == 'in' && m[1].to_i >= 59 && m[1].to_i <= 76)
  end
    ...
    data.split("\n\n").each do |p|
      ...
      p.split("\n").each do |line|
        line.split(' ').each do |field| 
          matched = field.match(/(.*):(.*)/)
          passport[matched[1].to_sym] = matched[2]
        end
      end
      passports << passport
    end
...
# puts AoC3.new(test_data).p2
# puts AoC3.new(test_data_2).p2
# puts AoC3.new(test_data_invalid).p2
puts AoC3.new(data).p1
